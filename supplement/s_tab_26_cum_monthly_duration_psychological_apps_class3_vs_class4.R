...
df_wide <- spread(df, key = Categories, value = Cumulative_duration_hours_monthly)

df_wide <- df_wide[, c("Case_ID", "Months_From_Earliest", "Class", "Psychological_capacity")]
...
catVars <- c("Class")
file_prefix <- "s_tab_26_cum_monthly_duration_psychological_apps_class3_vs_class4_month_"

dir.create(
  "~/Downloads/supplementary_table_26_output/",
  recursive = TRUE,
  showWarnings = FALSE
)
...
write.csv(
  merged_data,
  file = "~/Downloads/supplementary_table_26_cum_monthly_duration_psychological_apps_class3_vs_class4.csv",
  row.names = FALSE
)
