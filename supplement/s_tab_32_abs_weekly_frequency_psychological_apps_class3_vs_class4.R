...
df_wide <- spread(df, key = Categories, value = absolute_freq_weekly)
...
catVars <- c("Class")
file_prefix <- "s_tab_32_abs_weekly_frequency_psychological_apps_class3_vs_class4_week_"

dir.create(
  "~/Downloads/supplementary_table_32_output/",
  recursive = TRUE,
  showWarnings = FALSE
)
...
write.csv(
  merged_data,
  file = "~/Downloads/supplementary_table_32_abs_weekly_frequency_psychological_apps_class3_vs_class4.csv",
  row.names = FALSE
)
